...
// Short TTL — JWT lifetime bounds the true expiry. Mirrors
// `SESSION_CACHE_TTL_SECONDS` in jwt-auth.guard.ts so the guard's negative
// cache line and our poison share the same freshness horizon.
const REVOKE_CACHE_TTL_SECONDS = 60;
...
export class SignOutService {
  ...
  async signOut(user: AuthUser): Promise<void> {
    ...
    try {
      await this.valkey.setSession(jti, 'revoked', REVOKE_CACHE_TTL_SECONDS);
    } catch {
