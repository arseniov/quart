...
import { getActiveTraceContext, OtelEnvSchema, startOtel } from '../../src/observability/otel.js';

describe('OtelEnvSchema', () => {
  it('defaults to enabled, quart-api service, namespace=quart, port=9464, host=0.0.0.0, ratio=0.1', () => {
...
describe('startOtel', () => {
  ...
  it('returns a no-op handle when OTEL_ENABLED=false (no SDK instantiated)', () => {
  ...
  it('constructs NodeSDK with Resource, PrometheusExporter, ParentBasedSampler, auto-instr', () => {
  ...
  it('calls sdk.start() exactly once', () => {
    startOtel();
    const sdk = mocks.NodeSDKInstances[0]!;
    expect(sdk.start).toHaveBeenCalledOnce();
  });

  it('passes resource attributes from env (service.name, service.namespace, service.version, deployment.environment)', () => {
    startOtel({
      ...OtelEnvSchema.parse({}),
  ...
  it('wires PrometheusExporter with host/port from env', () => {
    startOtel({
      ...OtelEnvSchema.parse({}),
  ...
  it('passes parent-based sampler with TraceIdRatioBasedSampler root', () => {
    startOtel({ ...OtelEnvSchema.parse({}), OTEL_TRACES_SAMPLER_ARG: 0.25 });
    expect(mocks.TraceIdRatioInstances).toHaveLength(1);
  ...
  it('attaches getNodeAutoInstrumentations() to the SDK', () => {
    startOtel();
