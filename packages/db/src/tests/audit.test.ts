...
describe('computeRowHash', () => {
  it('uses GENESIS_PREV_HASH for the first row', () => {
    const h = computeRowHash({ prev_hash: GENESIS_PREV_HASH, payload_canonical_sha256: 'a'.repeat(64) });
  ...
  it('changes when prev_hash changes', () => {
    const a = computeRowHash({ prev_hash: 'a'.repeat(64), payload_canonical_sha256: 'z'.repeat(64) });
    const b = computeRowHash({ prev_hash: 'b'.repeat(64), payload_canonical_sha256: 'z'.repeat(64) });
    expect(a).not.toBe(b);
  });
});
